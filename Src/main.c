...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_heartbeat_time >= 10000) // Cada 500 ms (Se cambia el last_heartbeat_time para cambiar el led parpadee mas rapido)
    {
    ...
    if (button_flag == 1)
    {
      ...
      led_ext_off_time = HAL_GetTick() + 10000; // (Aqui se cambia el tiempo que demora el led en apagar)

      // 3. Enviar mensaje por UART
      sprintf(tx_buffer, "Ud a presionado un boton :) LED EXT ON.\r\n"); //(Se cambia el mensaje del boton presionado)
